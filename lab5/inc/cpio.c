...
// +---------+---------+---------+---------+---------+---------+---------+
// |		 |		   |		 |		   |		 |		   |		 |
// | 2(argc) |	**argv | argv[0] | argv[1] |  NULL   |		   |		 |
// |		 |		   |		 |		   |	     |		   |		 |
// +---------+---------+---------+---------+---------+---------+---------+
// 						  |			|                  |          |
// 						  +----------------------------+		  |
...
//	uart_printf("%d,%d",cnt1,cnt2);(4,18)
	int sum=8+8+8*cnt1+cnt2;	//argc, **argv, NULL
	ret=(ret-sum);
	//alignment
	ret=ret-(ret&15);

	char* tmp=(char*)ret;
	*(unsigned long*)tmp=cnt1-1; 	//set argc, cnt1-1(with null)
	tmp+=8;
	*(unsigned long*)tmp=(unsigned long)(tmp+8);	//**argv-->*argv[0]
	tmp+=8;											// *argv[0]
	char* buffer=tmp+8*cnt1;
	for(int i=0;i<cnt1;++i){
		if(i+1==cnt1){
			*(unsigned long*)tmp=0;							// NULL
		}else{
			*(unsigned long*)tmp=(unsigned long)buffer;		//*argv[]-->argv[][]
			tmp+=8;
			for(int j=0;;++j){
				*buffer=argv[i][j];
				buffer++;
