...
void shell(){
	...
	while(1){
		...
		if(strcmp(buffer,"help")==0){
			...
			vfs_close(f);
		}else if(strcmp(buffer,"lab7")==0){

			char buf[100];
			file* f=vfs_open("A.TXT",0);
			int n=vfs_read(f,buf,100);
			buf[n]=0;
			uart_printf("Before A.TXT: %s\n",buf);
			vfs_close(f);

			file* a=vfs_open("A.TXT",0);
			vfs_write(a,"Hello World!",12);
			vfs_close(a);

			file* b=vfs_open("A.TXT",0);
			n=vfs_read(f,buf,100);
			buf[n]=0;
			uart_printf(" After A.TXT : %s\n",buf);
			vfs_close(b);
